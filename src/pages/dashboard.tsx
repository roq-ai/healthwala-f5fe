...
import { getPersonalTrainers } from 'apiSdk/personal-trainers';
import { PersonalTrainerInterface } from 'interfaces/personal-trainer';
import { getDieticians } from 'apiSdk/dieticians';
import { DieticianInterface } from 'interfaces/dietician';
import { getHealthEquipments } from 'apiSdk/health-equipments';
import { HealthEquipmentInterface } from 'interfaces/health-equipment';

import { HealthEquipmentListPage as ShowCaseList } from 'pages/health-equipments';

function DashboardPage() {
  const { session } = useSession();

  const personalTrainerStat = useSWR<PaginatedInterface<PersonalTrainerInterface>>(
    () => `/personal-trainers`,
    () => getPersonalTrainers({ limit: 1, offset: 0 }),
  );

  const dieticianStat = useSWR<PaginatedInterface<DieticianInterface>>(
    () => `/dieticians`,
    () => getDieticians({ limit: 1, offset: 0 }),
  );

  const healthEquipmentStat = useSWR<PaginatedInterface<HealthEquipmentInterface>>(
    () => `/health-equipments`,
    () => getHealthEquipments({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Personal Trainers',
      result: personalTrainerStat?.data?.totalCount,
      isLoading: personalTrainerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Dieticians',
      result: dieticianStat?.data?.totalCount,
      isLoading: dieticianStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Health Equipments',
      result: healthEquipmentStat?.data?.totalCount,
      isLoading: healthEquipmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Health Equipments
                </Text>
                <NextLink href={`health-equipments`} passHref legacyBehavior>
                  <Button
