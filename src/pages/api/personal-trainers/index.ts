...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getPersonalTrainers() {
    ...
    const response = await prisma.personal_trainer
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'personal_trainer'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createPersonalTrainer() {
    ...
    const data = await prisma.personal_trainer.create({
      data: body,
    });
